...
Note that the behaviour and state are separate. Generally, components only contain state and they are stored in contiguous arrays for efficiency. Systems iterate through the components and apply transformations to the state to implement certain behaviours - like rendering or physics calculations. Storing the state this way minimizes the overall *cache misses* that might occur, which is often a bottleneck when processing a large amount of game characters.
